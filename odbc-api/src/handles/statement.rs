...
pub trait Statement: AsHandle {
    ...
    unsafe fn bind_input_parameter(
        ...
        if parameter_number < 5 {
            ...
            println!("xxxx odbc-api bind input parameter:{:?},parms_num:{:?},buff_len:{:?},value:{:?}",parameter_type,parameter_number,parameter.buffer_length(),value_byte);
            ...
            //println!("xxxx odbc-api bind input parameter old val ptr:{:?}, new ptr:{:?}, cdata_type:{:?}",val_ptr,value_ptr,parameter.cdata_type());
